...
if (isset($_GET['action']) && $_GET['action'] === 'print_payslip' && isset($_GET['month'])) {
    ...
    if ($total_required_hours == 0) {
        die("No work schedule hours found.");
    }

    $hourly_rate = $monthly_salary / $total_required_hours;

    // Get total worked hours
    $stmt = $conn->prepare("
        SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(time_out, time_in)))) AS total_time
        FROM attendance 
    ...
    <div style="text-align:center; margin-bottom:2px;">
        <img src="C:/xampp/htdocs/HSHR/images/LOGO123-modified.png" style="height:36px; margin-bottom:1px;">
   ...
   <!-- Salary Breakdown -->
    <table cellpadding="2" border="1" cellspacing="0" style="width:100%; margin-top:4px; font-size:6.5pt; border-collapse:collapse;">
        ...
        <tbody>
            ...
            <tr><td style="color:#888;">Required Hours</td><td align="right" style="color:#888;">' . number_format($total_required_hours, 2) . ' hrs</td></tr>
