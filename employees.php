...
<main class="wrapper">
        <div class="d-flex justify-content-start align-items-center">
            <h2 class="fw-bold mb-0">Employee Masterlist</h2>
            <button class="btn btn-primary ms-auto" data-bs-toggle="modal" data-bs-target="#addEmployeeModal">
                Add Employee
            </button>
        </div>
                    ...
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr class="text-dark">
                                ...
                                <td class="px-3 py-2"><?= htmlspecialchars($row["lastname"]) ?></td>
                                <td class="px-3 py-2"><?= htmlspecialchars($row["firstname"]) ?></td>
                                <td class="px-3 py-2"><?= htmlspecialchars($row["gender"]) ?></td>
                                <td class="px-3 py-2"><?= htmlspecialchars($row["email1"]) ?></td>
                                <td class="px-3 py-2">
                                    <span class="badge <?= $row["status"] == 'Active' ? 'bg-gradient-success' : 'bg-gradient-danger' ?>">
                                        <?= htmlspecialchars($row["status"]) ?>
                                    </span>
                                </td>
                                <td class="px-3 py-2 text-center">
                                    <a href="employee_details.php?id=<?= $row['id'] ?>" class="btn btn-outline-info btn-sm rounded-pill shadow-sm">
